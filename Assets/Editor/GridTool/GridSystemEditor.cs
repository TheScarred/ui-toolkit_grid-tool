...
public class GridSystemEditor : Editor
{
    ...
    private void OnEnable()
    {
        ...
        createGridButton = root.Q<Button>("createGridBTN");
        ...
        createGridButton.clicked += () => gridSystem.CreateGrid();
...
public override VisualElement CreateInspectorGUI()
    {
        ...
        for(int x = 0; x < gridSystem.Rows; x++)
        {
            ...
            for (int y = 0; y < gridSystem.Columns; y++)
            {
                ...
                if (gridSystem.GridDataFunction != null)
                {
                    CellData cellData = gridSystem.GridDataFunction.Cells.Find(item => item.Position == new Vector2(x, y));
                    if (cellData != null)
                    {
                ...
                column.RegisterCallback<ClickEvent>(e =>
                {
                    if (gridSystem.GridDataFunction == null)
                    {
                        ...
                        if(path != null)
                        {
                            GridData _gridData = GridData.CreateInstance(gridSystem.Rows, gridSystem.Columns);
                            gridData.value = _gridData;
                            gridSystem.GridDataFunction = _gridData;

                            AssetDatabase.CreateAsset(_gridData, path);
                            CellData cellData = _gridData.Cells.Find(item => item.Position == new Vector2(column.posxAttr, column.posyAttr));
                    ...
                    { 
                        CellData cellData = gridSystem.GridDataFunction.Cells.Find(item => item.Position == new Vector2(column.posxAttr, column.posyAttr));
                        cellData.Color = currentColor;
                        cellData.GridObject = currentGridObject;
                    }  
                    ...
                    EditorUtility.SetDirty(gridSystem.GridDataFunction);
